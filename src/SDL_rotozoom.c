...
SDL_Surface* rotateSurface90Degrees(SDL_Surface* pSurf, int numClockwiseTurns)
{
    ...
    SDL_Surface* pSurfOut  = SDL_CreateRGBSurface(
        pSurf->flags, newWidth, newHeight, pSurf->format->BitsPerPixel,
        pSurf->format->Rmask, pSurf->format->Gmask, pSurf->format->Bmask,
        pSurf->format->Amask);
    if(!pSurfOut) {
        return NULL;
    }
...
void rotozoomSurfaceSizeTrig(int width, int height, double angle, double zoomx,
