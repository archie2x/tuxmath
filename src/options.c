...
const char* const OPTION_TEXT[NUM_GLOBAL_OPTS + 1] = {
    "PER_USER_CONFIG", "USE_SOUND",  "MENU_SOUND", "MENU_MUSIC", "FULLSCREEN",
    "USE_KEYPAD",      "USE_IGLOOS", "USE_TTS",    "END_OF_OPTS"};

const int DEFAULT_GLOBAL_OPTS[NUM_GLOBAL_OPTS] = {1, 1, 1, 1, 1, 0, 1, 1};
...
int Opts_Initialize(void)
{
    ...
    {
        global_options =
            (global_option_type*)malloc(sizeof(global_option_type));
    }
    if (!global_options)
    {
        return 0;
...
void Opts_SetLessonTitle(char* title)
{
    if (title && title[0] != '\0')
    {
        strncpy(game_options->lesson_title, title,
                sizeof(game_options->lesson_title));
    }
}
...
void Opts_SetFontName(char* font_name)
{
    ...
    {
        strncpy(game_options->current_font_name, font_name,
                sizeof(game_options->current_font_name));
    }
}
...
void Opts_SetBonusSpeedRatio(float val)
{
    if (val < 1)
    {
        ...
        fprintf(
            stderr,
            "bonus_speed_ratio must be at least 1, resetting accordingly.\n");
    }
    if (val > MAX_BONUS_SPEED_RATIO)
    {
        val = MAX_BONUS_SPEED_RATIO;
        fprintf(stderr,
                "Warning: requested bonus_speed_ratio above maximum, setting "
                "to %g.\n",
...
void Opts_SetSpeed(float val)
{
    if (val < MINIMUM_SPEED)
    {
        val = MINIMUM_SPEED;
        fprintf(stderr,
                "Warning: requested speed below minimum, setting to %g.\n",
                MINIMUM_SPEED);
    }
    if (val > MAX_MAX_SPEED)
    {
        val = MAX_MAX_SPEED;
        fprintf(stderr,
                "Warning: requested speed above Tuxmath's maximum, setting to "
                "%g.\n",
                MAX_MAX_SPEED);
    }
    if (val > Opts_MaxSpeed())
    {
        val = Opts_MaxSpeed();
        fprintf(stderr,
                "Warning: requested speed above currently selected maximum, "
                "setting to %g.\n",
                Opts_MaxSpeed());
    }
...
void Opts_SetSpeedupFactor(float val)
{
    if (val < MIN_SPEEDUP_FACTOR)
    {
        ...
        fprintf(stderr,
                "Warning: requested speedup factor below Tuxmath's minimum, "
                "setting to %g.\n",
                MIN_SPEEDUP_FACTOR);
    }
    if (val > MAX_SPEEDUP_FACTOR)
    {
        ...
        fprintf(stderr,
                "Warning: requested speedup factor above Tuxmath's maximum, "
                "setting to %g.\n",
                MAX_SPEEDUP_FACTOR);
    }
...
void Opts_SetMaxSpeed(float val)
{
    if (val < MINIMUM_SPEED)
    {
        val = MINIMUM_SPEED;
        fprintf(stderr,
                "Warning: requested max speed below minimum, setting to %g.\n",
                MINIMUM_SPEED);
    }
    if (val > MAX_MAX_SPEED)
    {
        val = MAX_MAX_SPEED;
        fprintf(stderr,
                "Warning: requested max speed above Tuxmath's maximum, setting "
                "to %g.\n",
                MAX_MAX_SPEED);
    }
    if (val < Opts_Speed())
    {
        val = Opts_Speed();
        fprintf(stderr,
                "Warning: requested max speed less than current speed, setting "
                "to %g.\n",
                Opts_MaxSpeed());
    }
...
void Opts_SetStartingComets(int val)
{
    if (val < MIN_COMETS)
    {
        val = MIN_COMETS;
        fprintf(stderr,
                "Warning: requested starting comets below Tuxmath's minimum, "
                "setting to %d.\n",
                MIN_COMETS);
    }
    if (val > MAX_MAX_COMETS)
    {
        val = MAX_MAX_COMETS;
        fprintf(stderr,
                "Warning: requested starting comets above Tuxmath's maximum, "
                "setting to %d.\n",
                MAX_MAX_COMETS);
    }
    if (val > Opts_MaxComets())
    {
        val = Opts_MaxComets();
        fprintf(stderr,
                "Warning: requested starting comets above currently selected "
                "maximum, setting to %d.\n",
                Opts_MaxComets());
    }
...
void Opts_SetExtraCometsPerWave(int val)
{
    if (val < 0)
    {
        val = 0;
        fprintf(stderr,
                "Warning: requested extra comets below Tuxmath's minimum, "
                "setting to %d.\n",
                0);
    }
    if (val > MAX_MAX_COMETS)
    {
        val = MAX_MAX_COMETS;
        fprintf(stderr,
                "Warning: requested extra comets above Tuxmath's maximum, "
                "setting to %d.\n",
                MAX_MAX_COMETS);
    }
    if (val > Opts_MaxComets())
    {
        val = Opts_MaxComets();
        fprintf(stderr,
                "Warning: requested extra comets above currently selected "
                "maximum, setting to %d.\n",
                Opts_MaxComets());
    }
...
void Opts_SetMaxComets(int val)
{
    if (val < MIN_COMETS)
    {
        val = MIN_COMETS;
        fprintf(stderr,
                "Warning: requested max comets below Tuxmath's minimum, "
                "setting to %d.\n",
                MIN_COMETS);
    }
    if (val > MAX_MAX_COMETS)
    {
        val = MAX_MAX_COMETS;
        fprintf(stderr,
                "Warning: requested max comets above Tuxmath's maximum, "
                "setting to %d.\n",
                MAX_MAX_COMETS);
    }
...
void Opts_SetPowerupFreq(int val)
{
    ...
    else
    {
        fprintf(
            stderr,
            "Warning: requested powerup frequency illegal, setting to %d.\n",
...
void Opts_SetNextMission(char* str)
{
    ...
    else
    {
        fprintf(
            stderr,
            "Warning: Opts_SetNextMission() - string invalid or overflow\n");
    }
}
...
void Opts_SetDangerLevel(float val)
{
    if (val < 0)
    {
        ...
        fprintf(
            stderr,
            "Warning: danger level must be between 0 and 1, setting to 0.\n");
    }
    if (val > 1)
    {
        val = 1;
        fprintf(
            stderr,
            "Warning: danger level must be between 0 and 1, setting to 1.\n");
    }
...
void Opts_SetDangerLevelSpeedup(float val)
{
    ...
    {
        val = 1;
        fprintf(stderr, "Warning: danger_level_speedup must be at least 1, "
                        "setting to 1.\n");
    }
...
void Opts_SetDangerLevelMax(float val)
{
    ...
    {
        val = 0;
        fprintf(stderr, "Warning: danger level max must be between 0 and 1, "
                        "setting to 0.\n");
    }
    if (val > 1)
    {
...
void Opts_SetCityExplHandicap(float val)
{
    ...
    {
        val = 0;
        fprintf(stderr, "Warning: city_explode_handicap level set below "
                        "minimum, setting to 0.\n");
    }
...
void Opts_SetFPSLimit(int val)
{
    ...
    {
        val = 0;
        fprintf(stderr, "Warning: fps_limit level set below minimum, setting "
                        "to 0 (no limit).\n");
    }
...
int Opts_UsingSound(void)
{
    ...
    return (global_options->iopts[USE_SOUND] > 0 &&
            game_options->sound_hw_available);
}
...
void print_game_options(FILE* fp, int verbose)
{
    ...
    {
        fprintf(stderr,
                "print_game_options(): invalid game_option_type struct");
        return;
    }
    ...
    if (verbose)
    {
        fprintf(
            ...
            "# 1 = too dangerous, comets typically exploded at the moment they "
            "hit cities\n"
            "# Set it somewhere between these extremes. As a guideline, early\n"
            "# elementary kids might feel comfortable around 0.2-0.3, older "
            "kids\n"
