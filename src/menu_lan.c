...
int ConnectToServer(void)
{
    ...
    /* Draw heading: */
    {
        SDL_Surface* s = T4K_BlackOutline(_("Detecting servers"),
        ...
        s = T4K_BlackOutline(_("Please wait"), DEFAULT_MENU_FONT_SIZE, &white);
        if (s)
        {
            loc.x = (screen->w / 2) - (s->w / 2);
            loc.y = 140;
    ...
    while (!finished)
    {
        ...
        else if (servers_found ==
                 1) // One server - connect without player intervention
        {
            DEBUGMSG(debug_lan,
                     "Single server found - connecting automatically...");
        ...
        else if (servers_found > 1) // Multiple servers - ask player for choice
        {
            ...
            snprintf(
                buf, 256,
                _("TuxMath detected %d running servers.\nClick to continue..."),
                servers_found);
            ...
            T4K_CreateOneLevelMenu(MENU_SERVERSELECT, servers_found,
                                   servernames, NULL, "Server Selection", NULL);
            T4K_PrerenderMenu(MENU_SERVERSELECT);
            server_choice = T4K_RunMenu(
                MENU_SERVERSELECT, true, &DrawTitleScreen,
    ...
    // Now connected - get player nickname:
    {
         ...
         */
        snprintf(buf, 256, _("Connected to server: %s"),
                 LAN_ConnectedServerName());
        snprintf(buf2, 256, _("%s"), LAN_ConnectedServerLesson());
        NameEntry(player_name, buf, buf2,
                  _("Enter your name:")); // get nickname from user
...
int Pregame(void)
{
    ...
    SDL_Event event;
    SDL_Rect  title_rect,
        ready_rect; // NOTE stop_rect is a global from t4k/common.h (good
                    // idea???)
    ...
    // set up locations:
    ready_rect.x = screen->w * 0.45;
    ready_rect.y = screen->h * 0.15;
    ready_rect.w = ready_rect.h = screen->w * 0.1;
    // set up surfaces for remaining buttons:
    play_surf  = T4K_LoadImageOfBoundingBox("status/player_play.svg", IMG_ALPHA,
                                            ready_rect.w, ready_rect.h);
    pause_surf = T4K_LoadImageOfBoundingBox(
        "status/player_pause.svg", IMG_ALPHA, ready_rect.w, ready_rect.h);
    // set up surfaces for titles:
    ready_title       = T4K_BlackOutline(_("Waiting for other players"),
                                         DEFAULT_MENU_FONT_SIZE, &white);
    notready_title    = T4K_BlackOutline(_("Waiting until you are ready"),
                                         DEFAULT_MENU_FONT_SIZE, &white);
    ready_subtitle    = T4K_BlackOutline(_("Click \"Pause\" if not ready"),
                                         DEFAULT_MENU_FONT_SIZE, &white);
    notready_subtitle = T4K_BlackOutline(_("Click \"Play\" when ready"),
                                         DEFAULT_MENU_FONT_SIZE, &white);
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT,
                "%s. %s. %s. Press space to announce updated table",
                _("Waiting for other players"),
                _("Waiting until you are ready"),
                _("Click \"Play\" when ready"));
    ...
    if (widest < ready_title->w)
    {
        widest = ready_title->w;
    }
    if (widest < notready_title->w)
    {
        widest = notready_title->w;
    }
    if (widest < ready_subtitle->w)
    {
        widest = ready_subtitle->w;
    }
    if (widest < notready_subtitle->w)
    {
        widest = notready_subtitle->w;
    }
    ...
    while (status == PREGAME_WAITING)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            case SDL_EVENT_MOUSE_BUTTON_DOWN:
                /* "Stop" button - go to main menu: */
                {
                                        ...
                                        event.button.y))
                    {
                        ...
                        ready = !ready;
                        LAN_SetReady(ready); // tell server we are ready to
                                             // start
                        playsound(SND_TOCK);
                        break;
                    }
            ...
            case SDL_EVENT_KEY_DOWN:
            {
                ...
                {
                    ready = false;
                    LAN_SetReady(false); // tell server we are NOT ready to
                                         // start
                    playsound(SND_TOCK);
                    break;
                }
                default:
                {
                    // Do nothing - event. add support for toggle fullscreen,
                    // etc.
                }
                }
            }
        ...
        for (more_msgs = 1; more_msgs > 0; more_msgs = LAN_NextMsg(buf))
        {
            ...
            else if (strncmp(buf, "GAME_IN_PROGRESS",
                             strlen("GAME_IN_PROGRESS")) == 0)
            {
                status = PREGAME_GAME_IN_PROGRESS;
                break;
            }
            else if (strncmp(buf, "NETWORK_ERROR", strlen("NETWORK_ERROR")) ==
                     0)
            {
...
void draw_player_table(void)
{
    ...
    {
        int nr_width =
            T4K_SimpleText(_("Not Ready"), DEFAULT_MENU_FONT_SIZE, &white)->w;
        int text_h =
            T4K_SimpleText(_("Not Ready"), DEFAULT_MENU_FONT_SIZE, &white)->h;
        SDL_Rect shaded_loc;
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        ...
        if (LAN_PlayerConnected(i))
        {
            ...
            surf = T4K_BlackOutline(LAN_PlayerName(i), DEFAULT_MENU_FONT_SIZE,
                                    col);
            if (surf)
            {
                loc.x = name_x;
