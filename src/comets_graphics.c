    ...
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        if (comets[i].alive && !comets[i].bonus)
        {
            if (comets[i].expl == -1)
            {
                ...
                img = sprites[IMG_COMET]->frame[(FC_sprite_counter + i) %
                                                sprites[IMG_COMET]->num_frames];
    ...
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        if (comets[i].alive && comets[i].bonus)
        {
            if (comets[i].expl == -1)
            {
                answered = 0;
                /* Decide which image to display: */
                img = sprites[IMG_BONUS_COMET]
                          ->frame[(FC_sprite_counter + i) %
                                  sprites[IMG_BONUS_COMET]->num_frames];
                /* Display the formula (flashing, in the bottom half
                   of the screen) */
                if (comets[i].y < screen->h / 2 || FC_sprite_counter % 8 < 6)
                {
...
void comets_draw_cities(int igloo_vertical_offset, const cloud_type* cloud,
                        const city_type* cities, const penguin_type* penguins,
                        const steam_type* steam)
{
        ...
        do
        {
            for (i = 0; i < NUM_CITIES; i++)
            {
                if (cities[i].status != CITY_GONE &&
                    cities[i].layer > max_layer)
                {
                    max_layer = cities[i].layer;
                }
                if (penguins[i].status != PENGUIN_OFFSCREEN &&
                    penguins[i].layer > max_layer)
                {
                ...
                if (cities[i].layer == current_layer && cities[i].img != 0)
                {
                    ...
                    dest.y =
                        (screen->h) - (this_image->h) - igloo_vertical_offset;
                    if (cities[i].img == IMG_IGLOO_MELTED3 ||
                        cities[i].img == IMG_IGLOO_MELTED2)
                    {
                        dest.y -=
                            (images[IMG_IGLOO_MELTED1]->h - this_image->h) / 2;
                    }
                    dest.w = (this_image->w);
                    dest.h = (this_image->h);
                    ...
                    penguins[i].status != PENGUIN_OFFSCREEN)
                {
                        ...
                        penguins[i].status == PENGUIN_WALKING_ON)
                    {
                        ...
                        {
                            /* walking left */
                            this_image = flipped_images
                                [flipped_img_lookup[penguins[i].img]];
                            dest.x = penguins[i].x -
                                     images[IMG_PENGUIN_WALK_OFF2]->w / 2;
                        }
                        else
                        {
                            dest.x = penguins[i].x - this_image->w +
                                     images[IMG_PENGUIN_WALK_OFF2]->w /
                                         2; /* walking right */
                        }
                    }
                    else
                    {
                        dest.x = penguins[i].x - (this_image->w / 2);
                        dest.y = (screen->h) - (5 * (this_image->h)) / 4 -
        ...
        if (cloud->status == EXTRA_LIFE_ON)
        {
            ...
            for (i = 0; i < NUM_SNOWFLAKES; i++)
            {
                    ...
                    cloud->snowflake_y[i] < screen->h - igloo_vertical_offset)
                {
                    ...
                    dest.x =
                        cloud->snowflake_x[i] - this_image->w / 2 + cloud->x;
    ...
    else
    {
        ...
        for (i = 0; i < NUM_CITIES; i++)
        {
            ...
            {
                for (j = (FC_sprite_counter % 3); j < images[IMG_SHIELDS]->h;
                     j = j + 3)
                {
                    src.x = 0;
                    src.y = j;
...
void comets_draw_misc(MC_MathGame* curr_game, int wave, int extra_life_earned,
                      int bonus_comet_counter, int score,
                      int                 total_questions_left,
                      help_controls_type* help_controls)
{
    int      i;
    int      offset;
    SDL_Rect dest;
    ...
    if (mp_get_parameter(PLAYERS)) // && mp_get_parameter(MODE) == SCORE_SWEEP )
    {
        ...
        {
            snprintf(str, 64, "%s: %d", mp_get_player_name(i),
                     mp_get_player_score(i));
    ...
    if (Opts_LanMode())
    {
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            {
                snprintf(str, 64, "%s: %d", LAN_PlayerName(i),
                         LAN_PlayerScore(i));
                if (LAN_PlayerMine(i))
                {
...
void comets_draw_question_counter(MC_MathGame* curr_game,
                                  int          total_questions_left)
{
    int questions_left;
    int comet_img;
...
void comets_draw_led_console(MC_MathGame* curr_game, int neg_answer_picked,
                             int* digits)
{
    int      i;
    SDL_Rect src, dest;
    ...
    y = (screen->h - images[IMG_CONSOLE_LED]->h +
    ...
    for (i = -1; i < MC_MAX_DIGITS;
         i++) /* -1 is special case to allow minus sign */
...
void comets_draw_powerup(powerup_comet_type* powerup_comet)
{
    ...
    if (powerup_comet->comet.expl == -1)
    {
        ...
        img = sprites[imgid]
                  ->frame[FC_sprite_counter % sprites[imgid]->num_frames];
        if (!img)
        {
            return;
        }
    ...
    else
    {
        ...
        num_draw = 1;
        /* show each frame of explosion twice */
        img = sprites[IMG_POWERUP_COMET_EXPL]
                  ->frame[powerup_comet->comet.expl / 2];
    }
