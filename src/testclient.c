...
int main(int argc, char** argv)
{
    ...
    else // More than one server - will have to get player selection:
    {
        while (server_number < 0 || server_number >= servers_found)
        {
            ...
            print_server_list();
            fprintf(stderr,
                    "Enter the SERVER NUMBER you would like to connect to:\n");
...
int game_check_msgs(void)
{
    ...
    while (1)
    {
        ...
        if (strncmp(buf, "SEND_QUESTION", strlen("SEND_QUESTION")) == 0)
        {
            ...
            {
                fprintf(stderr,
                        "SEND_QUESTION received but could not add question\n");
            }
            else
            {
                // If we successfully added question, show new questions to
                // user:
        ...
        else if (strncmp(buf, "REMOVE_QUESTION", strlen("REMOVE_QUESTION")) ==
                 0)
        {
        ...
        else if (strncmp(buf, "TOTAL_QUESTIONS", strlen("TOTAL_QUESTIONS")) ==
                 0)
        {
    ...
    while (game_status == GAME_IN_PROGRESS)
    {
        ...
        if (read_stdin_nonblock(buf, NET_BUF_LEN))
        {
            ...
            if ((strncmp(buf, "quit", 4) == 0) ||
                (strncmp(buf, "exit", 4) == 0) || (strncmp(buf, "q", 1) == 0))
            {
            ...
            else
            {
                ...
                if ((fc != NULL))
                {
                    ...
                    // NOTE the '-1' means we aren't tracking times for
                    // testclient
    ...
    for (i = 0; i < QUEST_QUEUE_SIZE; i++)
    {
        if (comets[i].question_id != -1)
        {
            fprintf(stderr, "Comet %d - question %d:\t%s\n", i,
                    comets[i].question_id, comets[i].formula_string);
        }
        else
        {
...
MC_FlashCard* check_answer(int ans)
{
    ...
    for (i = 0; i < QUEST_QUEUE_SIZE; i++)
    {
        ...
        if ((comets[i].question_id != -1) && (comets[i].answer == ans))
        {
