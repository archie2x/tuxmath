...
This is a sort of interface-independent backend that could be used with a
different user interface. Developed as an enhancement to Bill Kendrick's "Tux of
Math Command" (aka tuxmath).  If tuxmath were a C++ program, this would be a C++
class.
...
enum
{
    ...
    COPIES_REPEATED_WRONGS, /* how many copies of an incorrectly answered
                               question to re-insert*/
    ALLOW_NEGATIVES,
    MAX_ANSWER,
    MAX_QUESTIONS,
    MAX_FORMULA_NUMS,
...
void         MC_SetOpt(MC_MathGame* game, unsigned int index,
                       int val); // access directly,for internal use
int          MC_GetOpt(MC_MathGame* game, unsigned int index);
void         MC_SetOp(MC_MathGame* game, const char* param,
                      int val); // access by text, for config reading
