...
void  clear_file(const char* path, const char* filename,
...
int main(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            printf(
                "\ntuxmathadmin version " ADMINVERSION
                ", Copyright (C) 2007 Tim Holy\n"
        ...
        else if (strcmp(argv[i], "--path") == 0)
        {
            ...
            {
                fprintf(stderr,
                        "%s option requires an argument (a directory name)\n",
                        argv[i]);
        ...
        else if (strcmp(argv[i], "--level") == 0)
        {
            ...
            {
                fprintf(stderr,
                        "%s option requires an argument (a level number)\n",
                        argv[i]);
                usage(EXIT_FAILURE, argv[0]);
            }
            else
        ...
        else if (strcmp(argv[i], "--createhomedirs") == 0)
        {
            ...
            {
                fprintf(stderr,
                        "%s option requires an argument (a file name)\n",
                        argv[i]);
                usage(EXIT_FAILURE, argv[0]);
            }
            else
    ...
    if (path == NULL)
    {
        fprintf(stderr, "Must have a valid path (either with --path or in the "
    ...
    if (is_confighighscores)
    {
        ...
        {
            fprintf(stderr,
                    "Must specify a level when configuring highscores\n");
            usage(EXIT_FAILURE, argv[0]);
        }
...
void usage(int err, char* cmd)
{
    ...
    fprintf(f,
            ...
            "       %s [--path <directory>] [--clearhighscores] "
            "[--cleargoldstars] [--consolidatelogs] [--clearlogs]\n"
            "\n",
            cmd, cmd, cmd, cmd);
...
void display_help(void)
{
    fprintf(
        ...
        "Examples:\n"
        "  tuxmathadmin --path /servervolume/tuxmath_users --createhomedirs "
        "users.csv\n"
        "  tuxmathadmin --createhomedirs users.csv\n"
        "    Creates a user directory tree in location "
        "/servervolume/tuxmath_users,\n"
        "    according to the structure specified in users.csv.  See "
        "configure.pdf\n"
        "    for details.  The second syntax is applicable if you've defined "
        "the\n"
        "    homedir path in the global configuration file.\n\n"
        "  tuxmathadmin --confighighscores --level 3\n"
        "    Sets up sharing of high scores at level 3 of the hierarchy (top "
        "is\n"
        "    level 1).  If students logging in are presented with a choice of "
        "grade,\n"
        "    then classroom, and then user, then level 1 is the school, level "
        "2 is the\n"
        "    grade, level 3 is the classroom, and level 4 is the individual "
        "student.\n"
        "    So level 3 would set it up so that all kids in the same classroom "
        "would\n"
        "    compete for high scores.\n\n"
        "  tuxmathadmin --unconfighighscores\n"
        "    Removes any existing highscores configuration.\n\n"
        "  tuxmathadmin --clearhighscores\n"
        "    Clears high scores for all users in the location specified by the "
        "homedir\n"
        "    setting in the global configuration file.\n\n"
        "  tuxmathadmin --path /servervolume/tuxmath_users/2ndgrade "
        "--clearhighscores\n"
        "    Clears the high scores for all users inside the 2ndgrade "
        "hierarchy.\n\n"
        "  tuxmathadmin --cleargoldstars\n"
        "    Clears the gold stars for all users.\n\n"
        "  tuxmathadmin --path /servervolume/tuxmath_users/1st\\ grade/Mrs.\\ "
        "Smith --cleargoldstars\n"
        "    Clears the gold stars for all users in Mrs. Smith's first grade "
        "class.\n\n"
        "  tuxmathadmin --consolidatelogs\n"
        "    Creates consolidated_log.csv files at one level above the lowest "
        "level\n"
        "    of the directory hierarchy. These files can be opened with a "
        "spreadsheet\n"
        "    program. This may be useful for tracking student progress.\n"
        "    Note also that each student has a personal log.csv file in "
        "his/her own\n"
        "    directory.\n\n"
    ...
    while (fgets(buf, PATH_MAX, fp))
    {
        ...
        if (max_depth == 0)
        {
            ...
            if (current_dirtree == NULL)
            {
                fprintf(
                    stderr,
                    "Error: couldn't allocate memory for directory tree.\n");
                exit(EXIT_FAILURE);
            }
            for (i = 0; i < max_depth; i++)
            {
                ...
                {
                    fprintf(stderr, "Error: couldn't allocate memory for "
                                    "directory tree.\n");
                    exit(EXIT_FAILURE);
                }
                else
                {
                    *(current_dirtree[i]) =
                        '\0'; // initialize with blank string
                }
            }
        }
        else
        {
            // Check that this line doesn't change the size of the directory
            // hierarchy
            if (this_line_total_depth != max_depth)
            {
                fprintf(
                    ...
                    "depths to the hierarchy than the previous setting (%d).\n",
                    buf, max_depth);
                exit(EXIT_FAILURE);
            }
        }
        ...
        *line_cur     = '\0'; // replace linefeed with terminal \0
        ...
        while (current_depth >= 0)
        {
            ...
            if (line_cur + 1 < line_cur_end)
            {
                ...
                memcpy(current_dirtree[current_depth], copy_start,
                       line_cur_end - copy_start + 1);
        ...
        if (strlen(fullpath) < PATH_MAX)
        {
            ...
            if (mkdir(fullpath) < 0)
            {
                ...
                {
                    fprintf(stderr,
                            "Error: One of the upper-level directories in:\n  "
                            "%s\ndoesn't exist.  Check the syntax of your "
                            "configuration file.\n",
                            fullpath);
                    exit(EXIT_FAILURE);
                }
                ...
                {
                    fprintf(stderr,
                            "Error: the device has no room available.\n");
                    exit(EXIT_FAILURE);
                }
                else
                {
                    ...
                    // handling
                    fprintf(stderr,
                            "Error: couldn't make directory %s:\nDo you have "
                            "write permission for this location?\nDo you need "
                            "to be root/administrator?\n",
                            fullpath);
            ...
            else
            {
                ...
                if (line_cur > line_begin)
                { // as long as not making in the root directory...a bad idea
                  // anyway!
                ...
                line_cur++; // line_cur now points to beginning of newest
                            // directory
                strncpy(buf, line_begin, PATH_MAX); // we don't need buf anymore
                buf[strlen(buf)] = '/'; // append directory separator
                len              = strlen(buf);
                strncpy(buf + len, USER_MENU_ENTRIES_FILENAME,
                        PATH_MAX - len - strlen(USER_MENU_ENTRIES_FILENAME));
...
void config_highscores(const char* path, int level)
{
    ...
    for (i = 0; i < n_dirs; i++)
    {
        if (directory_level[i] == level)
        {
            ...
            {
                fprintf(stderr,
                        "confighighscores: pathname %s truncated, exiting.\n",
                        buf);
                exit(EXIT_FAILURE);
            }
            fp = fopen(buf, "w");
...
void clear_highscores(const char* path)
{
    ...
    for (i = 0; i < n_dirs; i++)
    {
        ...
        {
            fprintf(stderr,
                    "clearhighscores: pathname %s truncated, exiting.\n", buf);
            exit(EXIT_FAILURE);
        }
        fp = fopen(buf, "r");
        if (fp)
    ...
    for (i = 0; i < n_dirs; i++)
    {
        ...
        if (strlen(buf) >= PATH_MAX - 1)
        {
            error(EXIT_FAILURE, 0,
                  "consolidatelogs: pathname %s is truncated, exiting.\n", buf);
        }
...
int directory_crawl(const char* path)
{
    ...
    while (!isdone)
    {
        ...
        isdone          = 1; // We'll be finished if we don't find any new
                             // user_menu_entries files
        for (i = 0; i < previous_length; i++)
        {
            ...
            if (directory_level[i] == current_level)
            {
                ...
                strncat(fullpath, USER_MENU_ENTRIES_FILENAME,
                        PATH_MAX - strlen(fullpath) - 1);
                fp = fopen(fullpath, "r");
                if (fp != NULL)
                {
                    // We found the user_menu_entries file, read it and add
                    // directories
                    while (fgets(buf, PATH_MAX, fp))
                    {
                        ...
                        directory[current_length] = (char*)malloc(
                            (strlen(directory[i]) + strlen(line_begin) + 2) *
                            sizeof(char));
                        if (directory[current_length] == NULL)
                        {
                            fprintf(stderr, "Memory allocation error in "
                                            "directory_crawl.\n");
                            exit(EXIT_FAILURE);
                        }
