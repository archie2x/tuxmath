...
extern MC_MathGame*
    lan_game_settings; // TODO Deepak:- see its effect and change it accordingly
...
struct threadID
{
    ...
    TCPsocket server_sock; /* Socket descriptor for server to accept client TCP
                              sockets. */
    IPaddress        ip;
    SDLNet_SocketSet client_set;
    struct client_type
        client[MAX_CLIENTS]; // TODO Deepak removed static from it as they can't
                             // be declared inside it. might result problem in
                             // future
    int                  num_clients;
    struct srv_game_type srv_game;
};
struct threadID slave_thread[2]; // TODO it might have to be replaced with a
                                 // pointer pointing to head of the stack when
...
int RunServer(int argc, char* argv[])
{
    ...
    fprintf(stderr,
            "Started tuxmathserver, waiting for client to connect:\n>\n");
...
int RunServer_pthread(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr, "In RunServer_pthread() - warning - could not append "
                        "'--ignore-stdin'\n");
    }
...
{
    StopSrvrGame(0); // FIXME Deepak its hard coded. StopServer is called from
                     // somewhere else
    quit = 1;
}
...
int setup_server(int thread_id_no)
{
    ...
    if (SDLNet_ResolveHost(&(slave_thread[thread_id_no].ip), NULL,
                           DEFAULT_PORT) < 0)
    {
        fprintf(stderr, "SDLNet_ResolveHost: %s\n", SDLNet_GetError());
        return 0;
    ...
    if (!(slave_thread[thread_id_no].server_sock =
              SDLNet_TCP_Open(&(slave_thread[thread_id_no].ip))))
    {
...
# ifdef HAVE_FCNTL
        ...
        {
            fprintf(stderr,
                    "No name entered within timeout, will use default: %s\n",
                    DEFAULT_SERVER_NAME);
        }
...
# else
        ...
        DEBUGMSG(
            debug_lan,
            "fnctl() not available, initially using default server name\n");
    ...
    // Zero out our client list:
    {
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            slave_thread[thread_id_no].client[i].game_ready =
                0; /* waiting for user to OK game start */
            strncpy(slave_thread[thread_id_no].client[i].name,
                    _("Await player name"), NAME_SIZE); /* no nicknames yet */
            slave_thread[thread_id_no].client[i].sock =
                NULL; /* sockets start out unconnected     */
            slave_thread[thread_id_no].client[i].score = 0;
        }
    }

    // Now open a UDP socket to listen for clients broadcasting to find the
    // server:
...
void cleanup_server(int thread_id_no)
{
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        ...
        {
            SDLNet_TCP_Close(
                slave_thread[thread_id_no]
                    .client[i]
                    .sock); // close all the client sockets one by one
            slave_thread[thread_id_no].client[i].sock =
                NULL; // So we don't segfault in case cleanup()
    ...
    if (slave_thread[thread_id_no].client_set != NULL)
    {
        SDLNet_FreeSocketSet(
            slave_thread[thread_id_no]
                .client_set); // releasing the memory of the client socket set
        slave_thread[thread_id_no].client_set =
            NULL; // this helps us remember that this set is not allocated
    }
...
void server_handle_command_args(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        {
            printf(
                "\n\"Tux, of Math Command Server\" version " VERSION
...
void check_UDP(int thread_id_no)
{
    ...
    if (recvd > 0)
    {
        ...
        if (strncmp((char*)in->data, "TUXMATH_CLIENT",
                    strlen("TUXMATH_CLIENT")) == 0)
        {
            UDPpacket* out;
            int        sent = 0;
            ...
            // between multiple servers on same network (e.g. "Mrs. Adams'
            // Class");
            out = SDLNet_AllocPacket(NET_BUF_LEN);
            snprintf(buf, NET_BUF_LEN, "%s\t%s\t%s", "TUXMATH_SERVER",
                     server_name, Opts_LessonTitle());
...
void update_clients(int thread_id_no)
{
    ...
    sockets_used =
        SDLNet_TCP_AddSocket(slave_thread[thread_id_no].client_set,
                             slave_thread[thread_id_no].client[slot].sock);
     ...
     * slave_thread[thread_id_no].client[i].sock socket */
    ...
    DEBUGCODE(debug_lan)
    {
        ...
        client_ip            = SDLNet_TCP_GetPeerAddress(
            slave_thread[thread_id_no].client[slot].sock);
...
int server_check_messages(int thread_id_no)
{
    ...
    else if (actives)
    {
        ...
        // NOTE this will only pick up the first message for each socket each
        // time check_messages() called - probably OK if we just get it next
        // time through.
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            if ((slave_thread[thread_id_no].client[i].sock != NULL) &&
                (SDLNet_SocketReady(slave_thread[thread_id_no].client[i].sock)))
            {
                ...
                if (SDLNet_TCP_Recv(slave_thread[thread_id_no].client[i].sock,
                                    buffer, NET_BUF_LEN) > 0)
                {
                    DEBUGMSG(debug_lan,
                             "buffer received from client %d is: %s\n", i,
                             buffer);
                ...
                {
                    fprintf(stderr,
                            "Client %d active but receive failed - apparently "
                            "disconnected\n>\n",
                            i);
        ...
        check_game_clients(thread_id_no); // APPARENTLY checking one more time
                                          // "just in case"???
        ...
        if (actives > ready_found)
        {
            fprintf(
                stderr,
                "Warning: SDLNet_CheckSockets() reported %d active sockets,\n"
...
void server_check_stdin(int thread_id_no)
{
    ...
    if (read_stdin_nonblock(buffer, NET_BUF_LEN))
    {
        ...
        else if (strncmp(buffer, "endgame", 7) ==
                 0) // stop game leaving server running
        {
            end_game(thread_id_no);
        }
        else
...
void remove_client(int thread_id_no, int i)
{
    ...
    for (j = 0; j < MAX_CLIENTS; j++)
    {
        ...
        {
            SDLNet_TCP_Send(slave_thread[thread_id_no].client[j].sock, buf,
                            strlen(buf) + 1);
        }
    }

    SDLNet_TCP_DelSocket(slave_thread[thread_id_no].client_set,
                         slave_thread[thread_id_no].client[i].sock);
...
// with each game in its own thread with its own socket set and mathcards
// instance.
...
void check_game_clients(int thread_id_no)
{
    ...
    if (game_in_progress)
    {
        ...
        {
            DEBUGMSG(debug_lan, "All the clients have left the game, setting "
                                "game_in_progress = 0.\n");
...
void handle_client_nongame_msg(int thread_id_no, int i, char* buffer)
{
    ...
    else if (strncmp(buffer, "PLAYER_NOT_READY", strlen("PLAYER_NOT_READY")) ==
             0)
    {
        slave_thread[thread_id_no].client[i].game_ready = 0;
        // Inform other clients:
...
int handle_client_game_msg(int thread_id_no, int i, char* buffer)
{
    ...
    else if (strncmp(buffer, "WRONG_ANSWER", strlen("WRONG_ANSWER")) ==
             0) /* Player answered the question incorrectly , meaning comet
                   crashed into a city or an igloo */
    {
    ...
    {
        slave_thread[thread_id_no].client[i].game_ready =
            0; /* Player quitting game but not disconnecting */
    }
...
void msg_socket_index(int thread_id_no, int i, char* buf)
{
    ...
    SDLNet_TCP_Send(slave_thread[thread_id_no].client[i].sock, buf,
                    NET_BUF_LEN);
}
...
void game_msg_next_question(int thread_id_no)
{
    ...
    if (!MC_NextQuestion(lan_game_settings, &flash))
    {
        ...
        DEBUGMSG(debug_lan,
                 "MC_NextQuestion() returned NULL - no questions available\n");
        return;
    }
...
void game_msg_exit(int thread_id_no, int i)
{
    fprintf(stderr, "LEFT the GAME : %s",
            slave_thread[thread_id_no].client[i].name);
    remove_client(thread_id_no, i);
}
...
void start_game(int thread_id_no)
{
    ...
    for (j = 0; j < MAX_CLIENTS; j++)
    {
        ...
        {
            fprintf(stderr,
                    "Warning - start_game() entered when someone not ready\n");
            return;
        }
    }
    ...
    for (j = 0; j < MAX_CLIENTS; j++)
    {
        if ((slave_thread[thread_id_no].client[j].game_ready == 1) &&
            (slave_thread[thread_id_no].client[j].sock != NULL))
        {
            if (SDLNet_TCP_Send(slave_thread[thread_id_no].client[j].sock, buf,
            ...
            else
            {
                fprintf(
                    stderr,
                    "in start_game() - failed to send to client %d, removing\n",
                    j);
    ...
    slave_thread[thread_id_no].srv_game.max_quests_on_screen =
        Opts_StartingComets();
    slave_thread[thread_id_no].srv_game.quests_in_wave =
        slave_thread[thread_id_no].srv_game.rem_in_wave =
            Opts_StartingComets() * 2;
    ...
    //   {
    //     /* no more questions available */
    //     fprintf(stderr, "MC_NextQuestion() returned NULL - no questions
    //     available\n"); return;
    //   }
...
void server_update_game(int thread_id_no)
{
    ...
    wait_time =
        SRV_QUEST_INTERVAL /
        pow(DEFAULT_SPEEDUP_FACTOR, slave_thread[thread_id_no].srv_game.wave);

    /* Send another question if there is room and enough time has elapsed: */
    if (now_time - last_time > wait_time)
    {
        if ((slave_thread[thread_id_no].srv_game.active_quests <
             slave_thread[thread_id_no].srv_game.max_quests_on_screen) &&
        ...
        slave_thread[thread_id_no].srv_game.active_quests <= 0)
    {
        ...
        slave_thread[thread_id_no].srv_game.rem_in_wave =
            slave_thread[thread_id_no].srv_game.max_quests_on_screen * 2;
        send_counter_updates(thread_id_no);
        DEBUGMSG(debug_lan,
                 "/nAdvance to wave %d\n"
                 "srv_game.max_quests_on_screen = %d\n"
...
int send_counter_updates(int thread_id_no)
{
    ...
    {
        char buf[NET_BUF_LEN];
        snprintf(buf, NET_BUF_LEN, "%s\t%d", "TOTAL_QUESTIONS",
                 total_questions);
        transmit_all(thread_id_no, buf);
    }
    ...
    {
        char buf[NET_BUF_LEN];
        snprintf(buf, NET_BUF_LEN, "%s\t%d", "WAVE",
                 slave_thread[thread_id_no].srv_game.wave);
        transmit_all(thread_id_no, buf);
    }
    return 1;
...
int send_player_updates(int thread_id_no)
{
    ...
    /* Count how many players are active and send number to clients: */
    {
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            if ((slave_thread[thread_id_no].client[i].game_ready == 1) &&
                (slave_thread[thread_id_no].client[i].sock != NULL))
            {
                connected_players++;
            }
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
        ...
        {
            char buf[NET_BUF_LEN];
            snprintf(buf, NET_BUF_LEN, "%s\t%d\t%d\t%s\t%d",
...
int add_question(int thread_id_no, MC_FlashCard* fc)
{
    ...
    snprintf(buf, NET_BUF_LEN, "%s\t%d\t%d\t%d\t%s\t%s\n", "ADD_QUESTION",
             fc->question_id, fc->difficulty, fc->answer, fc->answer_string,
             fc->formula_string);
    transmit_all(thread_id_no, buf);
    return 1;
}
...
{
    char buf[NET_BUF_LEN];
    snprintf(buf, NET_BUF_LEN, "%s\t%d\t%d", "REMOVE_QUESTION", quest_id,
             answered_by);
    transmit_all(thread_id_no, buf);
    return 1;
}
...
int transmit(int thread_id_no, int i, char* msg)
{
    ...
    if (SDLNet_TCP_Send(slave_thread[thread_id_no].client[i].sock, buf,
                        NET_BUF_LEN) < NET_BUF_LEN)
    {
