...
#define CITY_EXPL_START                                                        \
    (3 * 5) /* Must be mult. of 5 (number of expl frames)                      \
             */
#define ANIM_FRAME_START                                                       \
    (4 * 2) /* Must be mult. of 2 (number of tux frames)                       \
             */
...
static char* comets_music_filenames[NUM_MUSICS] = {
    "01_rush.ogg",    "02_on_the_edge_of_the_universe.ogg",
    "03_gravity.ogg", "game.mod",
    "game2.mod",      "game3.mod",
};
...
int comets_game(MC_MathGame* mgame)
{
    ...
    // Save this in a "file global" so we don't have to pass it to every
    // function:
    curr_game = mgame;
    ...
    if (Opts_GetGlobalOpt(FULLSCREEN) ==
        !(screen->flags & SDL_WINDOW_FULLSCREEN))
    {
        fprintf(stderr,
    ...
    do
    {
        ...
        if (paused)
        {
            ...
            T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT,
                        _("Game paused. Press escape or p to continue"));
            pause_game();
            paused = 0;
            start_tts_announcer_thread();
        }
        ...
        if (Opts_GetGlobalOpt(USE_SOUND))
        {
            ...
            {
                T4K_AudioMusicLoad(
                    comets_music_filenames[(rand() % NUM_MUSICS)],
                    T4K_AUDIO_PLAY_ONCE);
            }
        }
#endif
...
int comets_initialize(void)
{
    ...
    for (i = 0; i < NUM_CITIES; i++)
    {
        ...
        {
            cities[i].x =
                (((screen->w / (NUM_CITIES + 1)) * i) + ((images[img]->w) / 2));
        }
        else
        {
            cities[i].x =
    ...
    igloo_vertical_offset =
        images[IMG_CITY_BLUE]->h - images[IMG_IGLOO_INTACT]->h;
...
/*
   ...
   */
void game_set_start_message(const char* m1, const char* m2, const char* m3,
                            const char* m4)
{
...
void comets_handle_help(void)
{
    ...
    if (Opts_GetGlobalOpt(USE_SOUND))
    {
        if (!T4K_IsPlayingMusic())
        {
            T4K_AudioMusicLoad(comets_music_filenames[(rand() % NUM_MUSICS)],
                               T4K_AUDIO_PLAY_ONCE);
        }
    }
#endif
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s %s %s",
                ...
                _("penguins' igloos from the"), _("falling comets."));

    timer = 0;
    while ((timer += FC_time_elapsed) < 5 &&
           !(quit_help = help_renderframe_exit()))
        ; // wait 5 more secs
    if (quit_help)
    {
    ...
    while (comets[0].alive && (timer += FC_time_elapsed) < 7 &&
           !(quit_help = help_renderframe_exit()))
        ; // intentional empty body — advance comet via the conditions' side
          // effects
    {
        T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "2 + 1 = ?");
        if (quit_help)
    ...
    if (comets[0].alive == 1)
    {
        ...
        game_set_message(&s2, _("the answer to the math problem"), left_edge,
                         135);
        game_set_message(&s3, _("and hitting 'space' or 'enter'."), left_edge,
                         170);
        game_set_message(&s4, _("Try it now!"), left_edge, 225);

        T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND,
                    "%s %s %s %s 2 + 1 = ", _("Stop a comet by typing"),
                    _("the answer to the math problem"),
    ...
    help_controls.laser_enabled = 0;
    timer                       = 0;
    while ((timer += FC_time_elapsed) < 3 &&
           !(quit_help = help_renderframe_exit()))
        ; // wait 3 secs

    speed = 30;
    game_set_message(&s1, _("If an igloo gets hit by a comet,"), left_edge,
                     100);
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s %s %s %s %s",
                _("If an igloo gets hit by a comet,"),
                _("it melts. But don't worry: the"), _("penguin is OK!"),
                _("Just watch what happens:"), _("(Press a key to start)"));
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s 9",
                _("Notice the answer"));
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s %s %s",
                _("If it gets hit again, the"), _("penguin leaves."),
                _("(Press a key when ready)"));
    ...
    while (comets[0].alive && ((timer += FC_time_elapsed) < 3) &&
           !(quit_help = help_renderframe_exit()))
        ;
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "2 + 2 = %s",
                _("Zap it now!"));
    ...
    while (((timer += FC_time_elapsed) < 10) &&
           !(quit_help = help_renderframe_exit()))
        ;
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s %s",
                _("Fast-moving powerup comets"),
                _("earn you a secret weapon:"));
        ...
        ; // intentional empty body
    {
        ...
        T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s",
                    powerup_comet->comet.flashcard.formula_string);
    }
    ...
    T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%s %s %s %s",
                _("Quit at any time by pressing"),
                _("'Esc' or clicking the 'X'"), _("in the upper right corner."),
                _("Do it now, and then play!"));
...
void help_add_comet(const char* formula_str, const char* ans_str)
{
    ...
    strncpy(comets[0].flashcard.formula_string, formula_str,
            MC_MaxFormulaSize());
    strncpy(comets[0].flashcard.answer_string, ans_str, MC_MaxAnswerSize());
    if (comets[0].formula_surf)
    {
        SDL_DestroySurface(comets[0].formula_surf);
    }
    if (comets[0].answer_surf)
    {
        SDL_DestroySurface(comets[0].answer_surf);
    }
    comets[0].formula_surf = T4K_BlackOutline(
        comets[0].flashcard.formula_string, comet_fontsize, &white);
    comets[0].answer_surf = T4K_BlackOutline(comets[0].flashcard.answer_string,
                                             comet_fontsize, &white);
}
...
void comets_write_message(const game_message* msg)
{
    ...
    {
        surf =
            T4K_BlackOutline(_(msg->message), DEFAULT_HELP_FONT_SIZE, &white);
        if (surf)
        {
            rect.w = surf->w;
            rect.h = surf->h;
    ...
    /* Demo mode! */
    {
        ...
        if (picked_comet == -1 && (rand() % 10) < 3)
        {
            ...
            else
            {
                ...
                DEBUGMSG(debug_game,
                         "Demo mode, comet %d attacked with answer %d\n",
        ...
        /* Add a digit: */
        if (picked_comet != -1 && (FC_sprite_counter % 5) == 0 &&
            (rand() % 10) < 8)
        {
            tux_pressing = 1;
            ...
            else
            {
                /* "Press Return" */
                DEBUGMSG(debug_game,
                         "Demo mode firing with these digits: %d%d%d\n",
...
void comets_handle_answer(void)
{
    ...
    /* Register the indices of comets with correct answers in the
     * comets_answer[] array.  If smartbomb_firing, all answers are
     * automatically correct here
     */
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        if (comets[i].alive && comets[i].expl == -1 &&
            (smartbomb_firing || 0 == strncmp(comets[i].flashcard.answer_string,
                                              ans, MC_MAX_DIGITS + 1)))
        {
    ...
    /* powerup comet */
    if (powerup_comet->comet.alive &&
        strncmp(powerup_comet->comet.flashcard.answer_string, ans,
                MC_MAX_DIGITS + 1) == 0)
    {
        powerup_ans = 1;
    }
    ...
    if (num_zapped != 0 || powerup_ans)
    {
        ...
        for (i = 0; i < num_zapped; i++)
        {
            ...
            if (Opts_LanMode())
#ifdef HAVE_LIBSDL_NET
                LAN_AnsweredCorrectly(
                    comets[index_comets].flashcard.question_id, t);
#else
            {
            ...
            {
                MC_AnsweredCorrectly(
                    curr_game, comets[index_comets].flashcard.question_id, t);
            }
            ...
            else if (num_zapped > 1 &&
                     i == num_zapped - 1) // only play sounds once for group
            {
                playsound(SND_LASER);
                playsound(SND_SIZZLE);
            ...
            /* FIXME looks like it might score a bit differently based on screen
             * mode? */
            add_score(25 * comets[index_comets].flashcard.difficulty *
                      (screen->h - comets[index_comets].y + 1) / screen->h);
        }
        ...
        if (powerup_ans)
        {
...
#ifdef HAVE_LIBSDL_NET
                LAN_AnsweredCorrectly(
                    powerup_comet->comet.flashcard.question_id, t);
#else
            {
            } // Needed for compiler, even though this path can't occur
#endif
            ...
            {
                MC_AnsweredCorrectly(
                    curr_game, powerup_comet->comet.flashcard.question_id, t);
            }
        }
    }
    else
...
// FIXME might be simpler to store vertical position (and speed) in terms of
// time rather than absolute position, and determine the latter in
// comets_draw_comets()
void comets_handle_comets(void)
{
    ...
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        if (comets[i].alive)
        {
            ...
            /* NOTE y increment scaled to make game play similar at any
             * resolution */
            if (comets[i].bonus)
            {
                comets[i].y += FC_time_elapsed * speed *
                               Opts_BonusSpeedRatio() * city_expl_height /
                               (480 - images[IMG_CITY_BLUE]->h);
            }
            else /* Regular comet: */
            {
                comets[i].y += FC_time_elapsed * speed * city_expl_height /
                               (480 - images[IMG_CITY_BLUE]->h);
            }
            ...
            /* Did it hit a city? */
            if (comets[i].y >= city_expl_height && comets[i].expl == -1)
            /* Oh no - an igloo or city has been hit!        */
            {
                ...
                {
                    MC_NotAnsweredCorrectly(curr_game,
                                            comets[i].flashcard.question_id);
                }
                ...
                {
                    MC_AddTimeToList(curr_game,
                                     (float)(ctime - comets[i].time_started) /
                                         1000);
                }
                ...
                if (cities[this_city].hits_left)
                {
                    ...
                    else
                    {
                        if (cities[comets[i].city].hits_left == 2)
                        {
                            ...
                            cities[this_city].counter =
                                1; /* Will act immediately */
                        }
                        else
                        {
                ...
                /* If slow_after_wrong selected, set flag to go back to starting
                 * speed and */
    ...
    for (i = 0; i < NUM_CITIES; i++)
    {
        ...
        if (Opts_GetGlobalOpt(USE_IGLOOS))
        {
            if (cities[i].status == CITY_EVAPORATING)
            {
                ...
                cities[i].layer =
                    0; /* these have to be drawn below the penguin */
                cities[i].counter--;
                if (cities[i].counter == 0)
                {
                    cities[i].img--;
            ...
            else
            {
                ...
                {
                    cities[i].layer =
                        1; /* these have to be drawn above the penguin */
                    cities[i].img = IMG_IGLOO_MELTED1 + cities[i].hits_left;
                    /* If we're in the middle of an "explosion," don't switch to
                       the new igloo. Note the steam may have a different
                       counter than the igloo on this matter; the switch is
                       designed to occur halfway through the steam cloud. */
                    if (cities[i].status == CITY_EXPLODING)
                    {
                        cities[i].img++;
                    }
        ...
        else
        {
            ...
            {
                cities[i].img = (IMG_CITY_BLUE_EXPL5 -
                                 (cities[i].counter / (CITY_EXPL_START / 5)));
            }
            else
            {
...
void comets_handle_penguins(void)
{
    ...
    for (i = 0; i < NUM_CITIES; i++)
    {
        ...
        case PENGUIN_WALKING_ON:
            ...
            penguins[i].counter++;
            direction = 2 * (i < NUM_CITIES / 2) -
                        1; /* +1 for walk right, -1 for left */
        ...
        case PENGUIN_WALKING_OFF:
            ...
            {
                if (penguins[i].x - images[IMG_PENGUIN_WALK_OFF1]->w / 2 >=
                    screen->w)
                {
                    penguins[i].status = PENGUIN_OFFSCREEN;
                }
...
int check_extra_life(void)
{
    ...
    if (extra_life_earned)
    {
        ...
        {
            cloud.x =
                screen->w + images[IMG_CLOUD]->w / 2; /* come from the right */
        }
...
void comets_handle_extra_life(void)
{
    ...
    if (cloud.status == EXTRA_LIFE_ON)
    {
        ...
        else
        {
            ...
            igloo_top =
                screen->h - igloo_vertical_offset - images[IMG_IGLOO_INTACT]->h;
            ...
            else if (cities[cloud.city].hits_left == 0)
            {
                ...
                // FIXME: It's a hack to encode a blended igloo with a negative
                // number!
                ...
                {
                    num_below_igloo =
                        0; // Don't show progress until a few have fallen
                }
                cities[cloud.city].img =
                    -((float)(num_below_igloo) / NUM_SNOWFLAKES) *
                    NUM_BLENDED_IGLOOS;
            }
            if (cloud.snowflake_y[NUM_SNOWFLAKES - 1] >
                screen->h - igloo_vertical_offset)
            {
...
void comets_draw(void)
{
    ...
    for (i = 0; i < MAX_LASER; i++)
    {
        if (laser[i].alive > 0)
        {
            draw_line(screen, laser[i].x1, laser[i].y1, laser[i].x2,
                      laser[i].y2, 255 / ((LASER_START + 1) - laser[i].alive),
                      192 / ((LASER_START + 1) - laser[i].alive), 64);
        }
    }
    ...
    {
        SDL_BlitSurface(player_left_surf, NULL, T4K_GetScreen(),
                        &player_left_pos);
    }
#endif
...
int check_exit_conditions(void)
{
    ...
    // NOTE can't use this check in LAN mode because we don't know if the server
    // has questions left

    /* Need to get out if no comets alive and MathCards has no questions left in
     * list, */
    ...
    if (!Opts_LanMode())
    {
        if (!MC_ListQuestionsLeft(curr_game) && !num_comets_alive())
        {
            ...
            DEBUGMSG(debug_game, "ListQuestionsLeft() = %d ",
                     MC_ListQuestionsLeft(curr_game));
...
void comets_handle_game_over(int game_status)
{
    ...
    {
        fprintf(stderr,
                "Entering comets_handle_comets_over() - game status = %d\n",
                game_status);
    ...
    case GAME_OVER_WON:
    {
        ...
        // stop_tts_announcer_thread();
        T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT,
                    _("Mission Accomplished! You won!"));
    ...
    case GAME_OVER_LAN_WON:
    {
        ...
        qsort((void*)sorted_scores, MAX_CLIENTS, sizeof(lan_player_type),
              compare_scores);
        ...
        for (i = 0; i < MAX_CLIENTS; i++)
        {
            ...
            {
                T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, APPEND, "%d.\t%s: %d",
                            rank, sorted_scores[i].name,
        ...
        do
        {
            ...
            for (i = 0; i < MAX_CLIENTS; i++)
            {
                if (sorted_scores[i].connected)
                {
                    snprintf(str, 64, "%d.\t%s: %d", rank,
                             sorted_scores[i].name, sorted_scores[i].score);
                    rank++;
...
void reset_level(void)
{
    ...
    /* Clear all comets: */
    /* NOTE - we should not need to reset them to not alive, as the wave is
     * supposed to be over. In LAN game, we can clobber the first comet in the
     * new wave if the messages come out of order, so we just warn here instead
     * of resetting "alive" to 0 - DSB We initialize all the fields at the start
     * of the game independent of this function anyway.
     */
    ...
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        DEBUGCODE(debug_game)
        {
            ...
            {
                fprintf(stderr,
                        "Warning - changing wave but comet[%d] still alive "
                        "(could be OK in LAN mode)\n",
                        i);
            }
        }
        // comets[i].alive = 0;
    ...
    /* exceed maximum:                                         */
    {
        ...
        if (Opts_AllowSpeedup())
        {
            ...
            if (use_feedback)
            {
                ...
                danger_level =
                    1 - (1 - danger_level) / Opts_DangerLevelSpeedup();
                ...
                /* Check to see whether we have any feedback data. If not, skip
                 * it. */
                if (comet_feedback_number == 0)
                {
                    use_feedback =
                        0; /* No comets above living cities, skip feedback */

#ifdef FEEDBACK_DEBUG
                    fprintf(stderr,
                            "No feedback data available, aborting.\n\n");
#endif
                }
                else
                {
                    ...
                    comet_avg_height =
                        comet_feedback_height / comet_feedback_number;
...
#ifdef FEEDBACK_DEBUG
                    fprintf(stderr,
                            "  comet average height = %g, height differential "
                            "= %g.\n",
...
int add_comet(void)
{
    ...
    {
        SDL_DestroySurface(comets[com_found].formula_surf);
    }
    if (comets[com_found].answer_surf)
    {
        SDL_DestroySurface(comets[com_found].answer_surf);
    }
    comets[com_found].formula_surf = T4K_BlackOutline(
        comets[com_found].flashcard.formula_string, comet_fontsize, &white);
    comets[com_found].answer_surf = T4K_BlackOutline(
        comets[com_found].flashcard.answer_string, comet_fontsize, &white);
    //  num_comets_alive++;
...
void comets_mouse_event(SDL_Event event)
{
    ...
    if (event.button.button == SDL_BUTTON_LEFT &&
        event.button.x >= SMARTBOMB_ICON_X &&
        event.button.x <= SMARTBOMB_ICON_X + SMARTBOMB_ICON_W &&
        event.button.y >= SMARTBOMB_ICON_Y &&
        event.button.y <= SMARTBOMB_ICON_Y + SMARTBOMB_ICON_H)
    {
        if (smartbomb_alive)
        {
            smartbomb_activate();
    ...
    if (!((event.button.x >= (screen->w / 2) - (keypad_w / 2) &&
           event.button.x <= (screen->w / 2) + (keypad_w / 2) &&
           event.button.y >= (screen->h / 2) - (keypad_h / 2) &&
           event.button.y <= (screen->h / 2) + (keypad_h / 2))))
...
void comets_key_event(SDL_Keycode key, SDL_Keymod mod)
{
    ...
    /* support for negative answer input DSB */
             ...
             MC_GetOpt(
                 curr_game,
                 ALLOW_NEGATIVES)) /* do nothing unless neg answers allowed */
    {
    ...
    else if (key == SDLK_BACKSPACE || key == SDLK_CLEAR || key == SDLK_DELETE)
    {
        /* [BKSP]: Clear digits! */
        for (i = 0; i < MC_MAX_DIGITS; ++i)
        {
...
void comets_recalc_positions(int xres, int yres)
{
    ...
    for (i = 0; i < NUM_CITIES; ++i)
    {
        ...
        {
            cities[i].x =
                (((xres / (NUM_CITIES + 1)) * i) + ((images[img]->w) / 2));
            DEBUGMSG(debug_game, "%d,", cities[i].x);
        }
        else
        {
            cities[i].x =
                xres - (xres / (NUM_CITIES + 1) * (i - NUM_CITIES / 2) +
                        images[img]->w / 2);
            DEBUGMSG(debug_game, "%d,", cities[i].x);
        }
    ...
    for (i = 0; i < MAX_MAX_COMETS; ++i)
    {
        ...
        if (comets[i].formula_surf !=
            NULL) // for safety, but shouldn't occur if comet is alive
        {
            SDL_DestroySurface(comets[i].formula_surf);
            comets[i].formula_surf = T4K_BlackOutline(
                comets[i].flashcard.formula_string, comet_fontsize, &white);
        }
        if (comets[i].answer_surf != NULL)
        {
            SDL_DestroySurface(comets[i].answer_surf);
            comets[i].answer_surf = T4K_BlackOutline(
                comets[i].flashcard.answer_string, comet_fontsize, &white);
        }
    }
}
...
int powerup_add_comet(void)
{
    ...
    powerup_comet->comet.formula_surf = T4K_BlackOutline(
        powerup_comet->comet.flashcard.formula_string, comet_fontsize, &white);
    powerup_comet->comet.answer_surf = T4K_BlackOutline(
        powerup_comet->comet.flashcard.answer_string, comet_fontsize, &white);
...
void comets_handle_powerup(void)
{
    ...
    powerup_comet->comet.x +=
        powerup_comet->inc_speed * FC_time_elapsed * screen->w / 640;

    if (powerup_comet->comet.expl >= 0)
    {
        powerup_comet->comet.expl++;
        if (powerup_comet->comet.expl >=
            sprites[IMG_COMET_EXPL]->num_frames * 2)
        {
            powerup_comet->comet.alive = 0;
            powerup_comet->comet.expl  = -1;
            if (powerup_comet->comet.answer_surf)
    ...
    else
    {
        ...
        if (powerup_comet_running == 0)
        {
            if (Opts_LanMode())
#ifdef HAVE_LIBSDL_NET
                LAN_NotAnsweredCorrectly(
                    powerup_comet->comet.flashcard.question_id);
#else
            {
            }
#endif
            ...
            {
                MC_NotAnsweredCorrectly(
                    curr_game, powerup_comet->comet.flashcard.question_id);
            }
        }
    }
}
...
void comets_handle_net_msg(char* buf)
{
    ...
    else if (strncmp(buf, "ADD_QUESTION", strlen("ADD_QUESTION")) == 0)
    {
        ...
        {
            fprintf(stderr,
                    "ADD_QUESTION received but could not add question\n");
        }
        else
        {
    ...
    else if (strncmp(buf, "REMOVE_QUESTION", strlen("REMOVE_QUESTION")) == 0)
    {
        ...
        {
            DEBUGMSG(
                debug_game | debug_lan,
                "REMOVE_QUESTION received but could not remove question\n");
            DEBUGMSG(debug_game | debug_lan,
                     "(this is OK if it was answered by this player, as it was "
                     "removed already)\n");
        }
        else
        {
    ...
    else if (strncmp(buf, "MISSION_ACCOMPLISHED",
                     strlen("MISSION_ACCOMPLISHED")) == 0)
    {
        game_over_won = 1;
    }
    ...
    {
        DEBUGMSG(debug_game | debug_lan,
                 "Unrecognized message from server: %s\n", buf);
    }
}
...
int add_quest_recvd(char* buf)
{
    ...
    {
        fprintf(stderr,
                "add_quest_recvd() - was unable to add question from server\n");
    }
...
int lan_add_comet(MC_FlashCard* fc)
{
    ...
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        if (!comets[i].alive)
        {
            com_found = i;
            DEBUGMSG(debug_game | debug_lan,
                     "lan_add_comet(): free comet slot found = %d\n", i);
            break;
        }
    }
    ...
    if (-1 == com_found)
    {
        /* free comet slot not found - no comet added: */
        DEBUGMSG(debug_game | debug_lan,
                 "lan_add_comet() called but no free comet slot\n");
        DEBUGCODE(debug_game | debug_lan) print_current_quests();
        return 0;
    }
    ...
    // DEBUGMSG(debug_game|debug_lan, "bonus_comet_counter is
    // %d\n",bonus_comet_counter);
...
int remove_quest_recvd(char* buf)
{
    ...
    {
        DEBUGMSG(debug_game | debug_lan,
                 "remove_quest_recvd() - returning because buf is NULL\n");
        return 0;
    }
    p = strchr(buf, '\t');
    if (!p)
    {
        DEBUGMSG(debug_game | debug_lan,
                 "remove_quest_recvd() - returning because strchr() failed to "
                 "find first tab char\n");
        return 0;
    }
    p++;
    ...
    DEBUGMSG(debug_game | debug_lan,
             "remove_quest_recvd() for id = %d, answered by %d\n", id,
             answered_by);

    if (id < 1) // The question_id can never be negative or zero
    {
        DEBUGMSG(debug_game | debug_lan,
                 "remove_quest_recvd() - illegal question_id: %d\n", id);
        return 0;
    }

    if (answered_by == LAN_MyIndex()) // If we answered, it's already removed
    {
        DEBUGMSG(debug_game | debug_lan, "remove_quest_recvd() - answered and "
                                         "already removed by this client\n");
        return 0;
    }
    ...
    {
        DEBUGMSG(debug_game | debug_lan,
                 "remove_quest_recvd() - could not find comet with id = %d\n",
                 id);
        return 0;
    }

    DEBUGMSG(debug_game | debug_lan,
             "comet on screen found with question_id = %d\n", id);
...
void print_current_quests(void)
{
    ...
    for (i = 0; i < MAX_MAX_COMETS; i++)
    {
        if (comets[i].alive == 1)
        {
            fprintf(stderr, "Comet %d - question %d:\t%s\n", i,
                    comets[i].flashcard.question_id,
                    comets[i].flashcard.formula_string);
        }
    }
    ...
    //     quest_queue[i].question_id, quest_queue[i].formula_string);
    //   else
...
int tts_announcer(void* unused)
{
    ...
    while (1)
    {
        ...
        {
            T4K_Tts_say(DEFAULT_VALUE, DEFAULT_VALUE, INTERRUPT, "on wave %d!",
                        wave);
            SDL_Delay(20);
            T4K_Tts_wait();
            tts_announcer_switch = 1;
        ...
        if (powerup_comet->comet.alive)
        {
            T4K_Tts_say(DEFAULT_VALUE, 70, INTERRUPT, "%S",
                        convert_formula_to_sentence(
                            powerup_comet->comet.flashcard.formula_string));
            SDL_Delay(20);
            T4K_Tts_wait();
        }
            ...
            if (iter != 0)
            {
                ...
                for (i = 0; i < n_announce; i++)
                {
                    ...
                    if (comets[order[i]].alive)
                    {
                        ...
                        T4K_Tts_say(
                            rate, rate, INTERRUPT, "%S",
                            convert_formula_to_sentence(
                                comets[order[i]].flashcard.formula_string));
                        SDL_Delay(20);
                        T4K_Tts_wait();
                    }
                }
...
void start_tts_announcer_thread()
{
    ...
    tts_announcer_thread =
        SDL_CreateThread(tts_announcer, "tts_announcer", NULL);
}
