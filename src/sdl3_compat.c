...
SDL_Surface* rotozoomSurface(SDL_Surface* src, double angle, double zoom, int smooth)
{
    ...
    if (!dst) {
        if (free_rgba) SDL_DestroySurface(rgba);
        return NULL;
    }
        ...
        for (int x = 0; x < ow; x++) {
            ...
            if (smooth) {
                ...
                if (x1 >= 0 && x0 < sw && y1 >= 0 && y0 < sh) {
                    ...
                    double w01 = (1 - tx) * ty;
                    double w11 = tx * ty;
                    Uint32 p00 = (x0 >= 0 && y0 >= 0) ? sp[y0 * s_stride + x0] : 0;
                    Uint32 p10 = (x1 < sw && y0 >= 0) ? sp[y0 * s_stride + x1] : 0;
                    Uint32 p01 = (x0 >= 0 && y1 < sh) ? sp[y1 * s_stride + x0] : 0;
                    Uint32 p11 = (x1 < sw && y1 < sh) ? sp[y1 * s_stride + x1] : 0;
                    double r = ((p00 >>  0) & 0xff) * w00 + ((p10 >>  0) & 0xff) * w10
                             + ((p01 >>  0) & 0xff) * w01 + ((p11 >>  0) & 0xff) * w11;
                    double g = ((p00 >>  8) & 0xff) * w00 + ((p10 >>  8) & 0xff) * w10
                             + ((p01 >>  8) & 0xff) * w01 + ((p11 >>  8) & 0xff) * w11;
                    double b = ((p00 >> 16) & 0xff) * w00 + ((p10 >> 16) & 0xff) * w10
                             + ((p01 >> 16) & 0xff) * w01 + ((p11 >> 16) & 0xff) * w11;
                    double a = ((p00 >> 24) & 0xff) * w00 + ((p10 >> 24) & 0xff) * w10
                             + ((p01 >> 24) & 0xff) * w01 + ((p11 >> 24) & 0xff) * w11;
                    px = ((Uint32)r) | (((Uint32)g) << 8) | (((Uint32)b) << 16) | (((Uint32)a) << 24);
                }
            } else {
