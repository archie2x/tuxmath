...
// button_radius = 0.27; const int min_font_size = 8, default_font_size = 20,
// max_font_size = 33;
...
int handle_activity(int act, int param)
{
    ...
    T4K_OnResolutionSwitch(
        NULL); // in case an activity forgets to register its own resolution
               // switch handler, prevent insanity
    ...
    case RUN_INFO:
    {
        ...
        snprintf(msg, sizeof(msg),
                 _("TuxMath version %s is free and open-source!\nYou can help "
                   "make it better.\nSuggestions, artwork, and code are all "
                   "welcome!\nDiscuss TuxMath at "
                   "tuxmath-devel@list.sourceforge.net"),
                 VERSION);
...
int run_academy(void)
{
    ...
    while (chosen_lesson >= 0)
    {
        ...
        if (read_named_config_file(local_game,
...
int run_arcade(int choice)
{
    const char* arcade_config_files[5] = {"arcade/space_cadet", "arcade/scout",
                                          "arcade/ranger", "arcade/ace",
                                          "arcade/commando"};

    const int arcade_high_score_tables[5] = {CADET_HIGH_SCORE, SCOUT_HIGH_SCORE,
                                             RANGER_HIGH_SCORE, ACE_HIGH_SCORE,
                                             COMMANDO_HIGH_SCORE};
    ...
    if (choice < NUM_MATH_COMMAND_LEVELS)
    {
        ...
        if (read_named_config_file(local_game, arcade_config_files[choice]))
        {
            ...
            if (check_score_place(hs_table, Opts_LastScore()) <
                HIGH_SCORES_SAVED)
            {
...
int run_custom_game(void)
{
    ShowMessageWrap(DEFAULT_MENU_FONT_SIZE,
                    _("Edit 'options' file in you home directory to create "
                      "customized game!\n"
...
int run_factoroids(int choice)
{
    const int factoroids_high_score_tables[2] = {FACTORS_HIGH_SCORE,
...
int run_lan_host(void)
{
    ...
    /* We still have possibility that a server is running on this machine as a
     * standalone program or within another tuxmath instance, in which case we
     * still can't start our server because the port is already taken:
     */
     ...
     */
    /* our same process. The server will use the currently selected Mathcards
     * settings, */
...
# ifdef HAVE_PTHREAD_H
...

    {
        ...
        while (chosen_lesson >= 0)
        {
            ...
            /* Now read the selected file and play the "mission": */
            if (read_named_config_file(lan_game_settings,
                                       lesson_list_filenames[chosen_lesson]))
            {
                break;
            }
    ...
    DEBUGMSG(debug_lan,
             "About to launch RunServer_pthread() with:\n"
             "serv_argv[0] = %s\n"
             "serv_argv[1] = %s\n"
             "serv_argv[2] = %s\n",
...
# else
    ...
    DEBUGMSG(debug_lan,
             "About to launch RunServer_prog() with:\n"
             "serv_argv[0] = %s\n"
             "serv_argv[1] = %s\n"
             "serv_argv[2] = %s\n",
             serv_argv[0], serv_argv[1], serv_argv[2]);
...
int stop_lan_host(void)
{
    ...
    {
        ShowMessageWrap(DEFAULT_MENU_FONT_SIZE,
                        _("Cannot stop server until current game finishes."));
        return 0;
    }
    StopServer();
...
int run_lan_join(void)
{
    ...
    {
        ShowMessageWrap(DEFAULT_MENU_FONT_SIZE,
                        _("Sorry, could not connect to server."));
        DEBUGMSG(debug_menu | debug_lan,
                 _("Sorry, could not connect to server.\n"));
        return 0;
    }
...
#else
    ShowMessageWrap(DEFAULT_MENU_FONT_SIZE,
                    _("Sorry, this version built without network support"));
    DEBUGMSG(debug_menu | debug_lan,
             _("Sorry, this version built without network support.\n"));
    return 0;
#endif
...
void LoadMenus(void)
{
    ...
    // NOTE level_menu.xml doesn't exist, and as it's not being used I'm
    // skipping the load for now -Cheez
    /* difficulty menu */
...
int RunLoginMenu(void)
{
    ...
    T4K_CreateOneLevelMenu(MENU_LOGIN, n_users, user_names, NULL, title,
                           trailer_quit);
    ...
    while (n_users)
    {
        ...
        if (chosen_login < 0 || chosen_login == n_users)
        {
            ...
            if (level == 0)
            {
                ...
                // Clean up memory (prob. not necessary, but prevents Valgrind
                // errors!)
...
void RunMainMenu(void)
{
    ...
    /* lessons menu */
    DEBUGMSG(debug_menu,
             "RunMainMenu(): Generating lessons submenu. (%d lessons)\n",
             num_lessons);
    ...
    T4K_CreateOneLevelMenu(MENU_LESSONS, num_lessons, lesson_list_titles,
                           icon_names, NULL, "Back");
