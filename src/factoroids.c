...
/* TUXSHIP_DECEL controls "friction" - 1 means ship glides infinitely, 0 stops
 * it instantly */
...
static int trig[12] = {1024, 1014, 984, 935, 868, 784,
                       685,  572,  448, 316, 117, 0};

static int bonus      = -1;
static int bonus_time = BONUS_NOTUSED;

static const int prime_numbers[]     = {2, 3, 5, 7, 11, 13};
static const int prime_power_limit[] = {
    7, 4, 3, 3, 2, 2}; // custom calibrated power limits for extra "goodness"

static const int prime_next[] = {2,  2,  3,  5,  5,  7,  7,
                                 11, 11, 11, 11, 13, 13, 2};
static const int prime_prev[] = {13, 13, 13, 2, 3, 3,  3, 5,
                                 5,  7,  7,  7, 7, 11, 11};
...
static int FF_add_asteroid(int x, int y, int xspeed, int yspeed, int size,
                           int angle, int angle_speed, int fact_num, int a,
                           int b, int new_wave);
static int FF_destroy_asteroid(int i, float xspeed, float yspeed);

static int  AsteroidColl(int astW, int astH, int astX, int astY, int x, int y);
...
static int  game_mouseroto(SDL_Event event)
{
...
void factors(void)
{
    ...
    factoroids_draw(asteroid, &tuxship, laser, bonus, bonus_time, digits, wave,
                    score, num, tux_img, button_pressed);
    ...
    while (game_status == FF_IN_PROGRESS)
    {
        ...
        if (SDL_GetTicks() > bonus_time && bonus_time != -1)
        {
            bonus_time = 0;
        }
        ...
        factoroids_draw(asteroid, &tuxship, laser, bonus, bonus_time, digits,
                        wave, score, num, tux_img, button_pressed);
        ...
        if (Opts_UsingSound())
        {
            ...
            {
                T4K_AudioMusicLoad(game_music_filenames[(rand() % NUM_MUSICS)],
                                   T4K_AUDIO_PLAY_ONCE);
            }
        }
#endif
...
void fractions(void)
{
    ...
    while (game_status == FF_IN_PROGRESS)
    {
        ...
        if (Opts_UsingSound())
        {
            if (!T4K_IsPlayingMusic())
            {
                T4K_AudioMusicLoad(game_music_filenames[(rand() % 3)],
                                   T4K_AUDIO_PLAY_ONCE);
            }
        }
#endif
...
static int FF_init(void)
{
     ...
     */
    /* SDL_WM_GrabInput dropped — use SDL_SetWindowMouseGrab(window, true) if
     * needed */
    ;
...
static void FF_handle_ship(void)
{
    ...
    if (right_pressed)
    {
        ...
        tuxship.x1 =
            fast_cos(DEG_PER_ROTATION * -roto_speed) * tuxship.centerx -
            fast_sin(DEG_PER_ROTATION * -roto_speed) * tuxship.centery;
        tuxship.y1 =
            fast_sin(DEG_PER_ROTATION * -roto_speed) * tuxship.centerx +
    ...
    else if (left_pressed)
    {
        ...
        tuxship.y1 = fast_sin(DEG_PER_ROTATION * roto_speed * tuxship.centerx +
                              fast_cos(DEG_PER_ROTATION * roto_speed)) *
                     tuxship.centery;
    }
...
static void FF_handle_asteroids(void)
{
    ...
    for (i = 0; i < MAX_ASTEROIDS; i++)
    {
        if (asteroid[i].alive)
        {
            ...
            asteroid[i].angle +=
                ROUND(asteroid[i].angle_speed * FC_time_elapsed);
            ...
            if (AsteroidColl(surf->w, surf->h, asteroid[i].x, asteroid[i].y,
                             tuxship.centerx, tuxship.centery) &&
...
int AsteroidColl(int astW, int astH, int astX, int astY, int x, int y)
{
    int astWq = astW / 8;
    int astHq = astH / 8;
    int x1, y1, x2, y2;
...
static void FF_add_level(void)
{
    ...
    for (i = 0; i < NUM_ASTEROIDS && NUM_ASTEROIDS < MAX_ASTEROIDS; i++)
    {
        ...
        {
            FF_add_asteroid(x, y, xvel, yvel, rand() % 2, rand() % 360,
                            rand() % 150 - 75, generatenumber(wave), 0, 0, 1);
        }
        else if (FF_game == FRACTIONS_GAME)
        {
            FF_add_asteroid(x, y, xvel, yvel, rand() % 2, rand() % 360,
                            rand() % 150 - 75, 0, (rand() % (31 + (wave * 2))),
                            (rand() % (80 + (wave * wave))), 1);
        }
    }
    ...
    if (wave != 1)
    {
        ...
        while ((timer += FC_time_elapsed) < 1)
        {
            ...
            factoroids_draw(asteroid, &tuxship, laser, bonus, bonus_time,
                            digits, wave, score, num, tux_img, button_pressed);
...
static int FF_over(int game_status)
{
    ...
    case FF_OVER_OTHER:
    {
        DEBUGMSG(debug_factoroids,
                 "Loop exited with FF_OVER_LOST or FF_OVER_OTHER\n");
    ...
    {
        DEBUGMSG(debug_factoroids,
                 "Loop exited with unrecognized status value: %dn",
                 game_status);
    }
    }
...
static int generatenumber(int wave)
{
    ...
    else
    {
        ...
        {
            DEBUGMSG(debug_factoroids,
                     "generatenumber() - wrn - invalid number: %d\n", n);
        }
    ...
    for (i = 0; i < MAX_LASER; i++)
    {
        if (laser[i].alive == 0)
        {
            ...
            for (k = 0; k < MAX_ASTEROIDS; k++)
            {
                ...
                asteroid_image =
                    get_asteroid_image(asteroid[k].size, asteroid[k].angle);
                dx = asteroid[k].x + asteroid_image->w / 2 - laser[i].x;
                dy = asteroid[k].y + asteroid_image->h / 2 - laser[i].y;
                // Find distance along laser of closest approach to asteroid
                // center
                s = dx * ux + dy * uy;
                if (s >= 0) // don't worry about it if it's in the opposite
                            // direction! (i.e., behind the ship)
                {
                    // Find the distance to the asteroid center at closest
                    // approach
                    dx2    = dx - s * ux;
                    dy2    = dy - s * uy;
                    d2     = dx2 * dx2 + dy2 * dy2;
                    ...
                    if (d2 < thresh)
                    {
                        ...
                        if ((FF_game == FACTOROIDS_GAME &&
                             (asteroid[k].isprime &&
                              ((num == asteroid[k].fact_number) ||
                               (num == 0)))) ||
                            (FF_game == FRACTIONS_GAME &&
                             (asteroid[k].isprime && num == 0)))
                        {
                            // It's valid, check to see if it's closest
                            if (s < smin)
                            {
                                // It's the closest yet examined but has not
                                // score
                                smin     = s;
                                zapIndex = k;
                                zapScore = 0;
                        ...
                        else if ((FF_game == FACTOROIDS_GAME && num > 1 &&
                                  ((asteroid[k].fact_number % num) == 0) &&
                                  (num != asteroid[k].fact_number)) ||
                                 (FF_game == FRACTIONS_GAME && num > 1 &&
                                  ((asteroid[k].a % num) == 0) &&
                                  ((asteroid[k].b % num) == 0) &&
...
static int FF_add_asteroid(int x, int y, int xspeed, int yspeed, int size,
                           int angle, int angle_speed, int fact_number, int a,
                           int b, int new_wave)
{
    int i;
    for (i = 0; i < MAX_ASTEROIDS; i++)
    {
        if (asteroid[i].alive == 0)
        {
            ...
            asteroid[i].y =
                (asteroid[i].ry -
                 tuxship_img_h(asteroid[i].angle / DEG_PER_ROTATION) / 2);
            asteroid[i].x =
                (asteroid[i].rx -
                 tuxship_img_w(asteroid[i].angle / DEG_PER_ROTATION) / 2);
            asteroid[i].xdead     = 0;
            asteroid[i].ydead     = 0;
            asteroid[i].isdead    = 0;
            asteroid[i].countdead = 0;

            // Without this on higher frame rates moevement in pixels per frame
            ...
            else if (FF_game == FRACTIONS_GAME)
            {
                ...
                asteroid[i].isprime =
                    is_simplified(asteroid[i].a, asteroid[i].b);
            }
            ...
            if (new_wave)
            {
                if (tuxship.x - 50 < asteroid[i].x + 80 &&
                    ...
                    tuxship.y + 50 > asteroid[i].y && tuxship.lives > 0 &&
                    asteroid[i].alive)
                {
            ...
            if (asteroid[i].isprime)
            {
                asteroid[i].size = 0;
                asteroid[i].centerx =
                    (images[IMG_ASTEROID1]->w / 2) + asteroid[i].x;
                asteroid[i].centery =
                    (images[IMG_ASTEROID1]->h / 2) + asteroid[i].y;
            ...
            else if (!asteroid[i].isprime)
            {
                asteroid[i].size = 1;
                asteroid[i].centerx =
                    (images[IMG_ASTEROID2]->w / 2) + asteroid[i].x;
                asteroid[i].centery =
                    (images[IMG_ASTEROID2]->h / 2) + asteroid[i].y;
                asteroid[i].radius = (images[IMG_ASTEROID1]->h / 2);
            }
    ...
    if (asteroid[i].alive == 1)
    {
        ...
        if (asteroid[i].size > 0)
        {
            ...
            if (num != 0)
            {
                ...
                if (FF_game == FACTOROIDS_GAME)
                {
                    FF_add_asteroid(asteroid[i].rx, asteroid[i].ry,
                                    asteroid[i].xspeed + (xspeed - yspeed) / 2,
                                    asteroid[i].yspeed + (yspeed + xspeed) / 2,
                                    0, rand() % 360, rand() % 150 - 75,
                                    (int)(asteroid[i].fact_number / num), 0, 0,
                                    0);

                    FF_add_asteroid(asteroid[i].rx, asteroid[i].ry,
                                    asteroid[i].xspeed + (xspeed + yspeed) / 2,
                                    asteroid[i].yspeed + (yspeed - xspeed) / 2,
                                    0, rand() % 360, rand() % 150 - 75, num, 0,
                                    0, 0);
                }
                else if (FF_game == FRACTIONS_GAME)
                {
                    FF_add_asteroid(asteroid[i].rx, asteroid[i].ry,
                                    ...
                                    (asteroid[i].yspeed + yspeed), 0,
                                    rand() % 360, rand() % 150 - 75, 0,
                                    (int)(asteroid[i].a / num),
                                    (int)(asteroid[i].b / num), 0);

                    FF_add_asteroid(asteroid[i].rx, asteroid[i].ry,
                                    (asteroid[i].xspeed + xspeed),
                                    ((asteroid[i].yspeed + yspeed) / 2), 0,
                                    rand() % 360, rand() % 150 - 75, 0,
                                    (int)(asteroid[i].b / num),
                                    (int)(asteroid[i].a / num), 0);
                }
            }
        }
...
void game_handle_user_events(void)
{
    ...
    while (SDL_PollEvent(&event) > 0)
    {
        ...
        if (event.type == SDL_EVENT_MOUSE_MOTION)
        {
            ...
            if (event.motion.x <= 10 || event.motion.x >= (screen->w - 10) ||
                event.motion.y <= 10 || event.motion.y >= (screen->h - 10))
            {
                mouse_reset = 1;
                SDL_WarpMouseInWindow(T4K_GetWindow(), screen->w / 2,
                                      screen->h / 2);
                continue;
            }
        ...
        if (event.type == SDL_EVENT_KEY_DOWN || event.type == SDL_EVENT_KEY_UP)
        {
            ...
            if (event.type == SDL_EVENT_KEY_DOWN)
            {
                ...
                // TODO this code only deals with the first 6 primes, we'd
                // probably want a more generic solution
                    ...
                    bonus_time == -1)
                {
                    ...
                    // special handling for the powerbomb, since it happens "at
                    // once"
                ...
                //&& MC_AllowNegatives())  /* do nothing unless neg answers
                // allowed */
                {
...
#ifdef JOY_YES
        else if (event.type == SDL_EVENT_JOYSTICK_BUTTON_DOWN && player_alive)
        {
...
{
    SDL_Event event = {/* SDL_NOEVENT dropped */ 0};
