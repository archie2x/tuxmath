...
static int FF_init(void)
{
     ...
     */
    /* SDL_WM_GrabInput dropped — use SDL_SetWindowMouseGrab(window, true) if needed */;
...
void game_handle_user_events(void)
{
    ...
    while (SDL_PollEvent(&event) > 0)
    {
        ...
        if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN || event.type == SDL_EVENT_MOUSE_BUTTON_UP)
        {
            ...
            event.key.key = key;
            event.type = state == true ? SDL_EVENT_KEY_DOWN : SDL_EVENT_KEY_UP;
        }
        if(event.type == SDL_EVENT_MOUSE_MOTION) {
            ...
            {
                mouse_reset = 1;
                SDL_WarpMouseInWindow(t4k_window, screen->w/2, screen->h/2);
                continue;
            }
...
static int game_mouse_event(SDL_Event event)
{
    ...
    else if(event.button.button == /* SDL_BUTTON_WHEELUP — handle SDL_EVENT_MOUSE_WHEEL */ -1) return CTRL_NEXT;
    else if(event.button.button == /* SDL_BUTTON_WHEELDOWN — handle SDL_EVENT_MOUSE_WHEEL */ -1) return CTRL_PREV;
...
void wait_for_input(void)
{
    ...
    while(1)
    {
        if(!SDL_PollEvent(&event))
        {
            ...
            /* SDL_EventState dropped */ (void)(SDL_EVENT_JOYSTICK_AXIS_MOTION, 0);

            SDL_WaitEvent(&event);

            /* SDL_EventState dropped */ (void)(SDL_EVENT_MOUSE_MOTION, 1);
            /* SDL_EventState dropped */ (void)(SDL_EVENT_JOYSTICK_AXIS_MOTION, 1);
        }
