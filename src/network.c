...
int LAN_DetectServers(void)
{
    ...
    // Here we will need to send every few seconds until we hear back from
    // server and get its ip address:  IPaddress bcast_ip;
    ...
    while (!done)
    {
        ...
        {
            DEBUGMSG(debug_lan,
                     "broadcast failed - network inaccessible.\nTrying "
        ...
        {
            if (strncmp((char*)in->data, "TUXMATH_SERVER",
                        strlen("TUXMATH_SERVER")) == 0)
            {
                done = 1;
...
int LAN_NextMsg(char* buf)
{
    ...
    else if (numready > 0)
    {
        ...
        if (SDLNet_SocketReady(sd))
        {
            ...
            {
                DEBUGMSG(debug_lan,
                         "In get_next_msg(), SDLNet_TCP_Recv() failed!\n");
        ...
        {
            DEBUGMSG(debug_lan, "In get_next_msg(), socket set reported active "
                                "but no activity found\n");
            SDLNet_TCP_DelSocket(set, sd);
            if (sd != NULL)
            {
                SDLNet_TCP_Close(sd);
...
char* LAN_PlayerName(int i)
{
    ...
    {
        fprintf(stderr,
                "Warning - invalid index %d passed to LAN_PlayerName()\n", i);
        return NULL;
    }
...
bool LAN_PlayerMine(int i)
{
    ...
    {
        fprintf(stderr,
                "Warning - invalid index %d passed to LAN_PlayerMine()\n", i);
        return false;
    }
...
bool LAN_PlayerReady(int i)
{
    ...
    {
        fprintf(stderr,
                "Warning - invalid index %d passed to LAN_PlayerReady()\n", i);
        return false;
    }
...
bool LAN_PlayerConnected(int i)
{
    ...
    {
        fprintf(stderr,
                "Warning - invalid index %d passed to LAN_PlayerConnected()\n",
                i);
        return false;
    }
...
int LAN_PlayerScore(int i)
{
    ...
    {
        fprintf(stderr,
                "Warning - invalid index %d passed to LAN_PlayerScore()\n", i);
        return -1;
    }
...
void intercept(char* buf)
{
    ...
    else if (strncmp(buf, "CONNECTED_PLAYERS", strlen("CONNECTED_PLAYERS")) ==
             0)
    {
        connected_players_recvd(buf);
        snprintf(buf, NET_BUF_LEN, "%s", "LAN_INTERCEPTED");
    }
    else if (strncmp(buf, "UPDATE_PLAYER_INFO", strlen("UPDATE_PLAYER_INFO")) ==
             0)
    {
